...
//        "bFilter": false,
//
//        "sAjaxSource": "mesChart.do?showMessListFromChart&code="+code+"&codeType="+codeType+"&dateType="+dateType+"&currentTime="+currentTime+"&processStatus="+processStatus+"&errLevel="+errLevel,
//        "bProcessing": true,
...
//
//            if (aData['processStatus'] == 0) {
//                 $('td:eq(5)', nRow).html("<span class=\"label label-sm label-danger\"> 未处理 </span>");
//             }else if (aData['processStatus'] == 1) {
//                 $('td:eq(5)', nRow).html("<span class=\"label label-sm label-warning\"> 正在处理 </span>");
//             }else if (aData['processStatus'] == 2) {
//                 $('td:eq(5)', nRow).html("<span class=\"label label-sm label-success\"> 已处理 </span>");
//             }
//
//        	 if (aData['errLevel'] == 1) {
//                 $('td:eq(6)', nRow).html("<span class=\"label label-sm label-warning\"> 警告 </span>");
//             }else if (aData['errLevel'] == 2) {
//                 $('td:eq(6)', nRow).html("<span class=\"label label-sm label-danger\"> 异常</span>");
//             }
//        	 if(aData['processStatus']==0){
//        		 $('td:eq(8)', nRow).html("<button class='btn btn-info btn-xs' onclick='processMes(\""+ aData['errId'] + "\")'>处理</button>");
//
//        	 }
//        }
//
...
function getMessageListNoPage(pageTurnFlag) {
    ...
    codeType=(codeType=='null'||codeType==null)?'':codeType;
    dateType=(dateType=='null'||dateType==null)?'':dateType;
    currentTime=(currentTime=='null'||currentTime==null)?'':currentTime;
    ...
    if(pageTurnFlag=='null'||pageTurnFlag==null){
        pageTurnFlag="";
        startRow="";
    }
    $.ajax({
        url:"mesChart.do?showMessListFromChart&code="+code+"&codeType="+codeType+"&dateType="+dateType+"&currentTime="+currentTime+"&processStatus="+processStatus+"&errLevel="+errLevel
                +"&pageTurnFlag="+pageTurnFlag+"&startRow="+startRow+"&startTime="+startTime+"&endTime="+endTime,
        type : 'post',
        cache : false,
        async : true,
        timeout : 1000 * 30,
