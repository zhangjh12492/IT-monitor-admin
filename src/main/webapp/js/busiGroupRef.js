...
busiTable = $("#busi").dataTable({
    "sAjaxSource": "/busiGroup.do?getBusiGroupList",
    "bProcessing": true,
...
$('#busi').on('click', 'tbody tr', function () {
    // 设置选中样式
...
//        usersTable.fnClearTable();
    ...
    groupEditTable = $("#groupEdit").dataTable({
            ...
            "bFilter": false,
            "bSort": true,
            "sAjaxSource": "/sysGroup.do?getNotExistGroup&sysInfoId=" + groupId,
            "bProcessing": true,
            "searching": false, //去掉搜索框
            "bLengthChange": false,// 是否允许自定义每页显示条数.
            "aLengthMenu": [
...
function getUsersTable(groupId) {
    ...
    if (usersTable != null) {
//        usersTable.fnClearTable();
        $('#groups').dataTable().fnDestroy();
    }
    usersTable = $("#groups").dataTable({
        "bDestory": true,
        "bFilter": false,
        "bSort": true,
