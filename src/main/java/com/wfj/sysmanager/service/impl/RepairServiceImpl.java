...
public class RepairServiceImpl extends BaseServiceImpl implements RepairServiceI {
	...
	private void repairMenuLevel2(Symenu root) {
		Symenu wangzhangguanli = new Symenu();
		wangzhangguanli.setId("wangzhangguanli");
		wangzhangguanli.setText("网站管理");
		wangzhangguanli.setSrc("");
		wangzhangguanli.setSymenu(root);
		wangzhangguanli.setSeq(BigDecimal.valueOf(1));
		menuDao.saveOrUpdate(wangzhangguanli);
		repairMenuWzGl(wangzhangguanli);// 修复网站管理子菜单
    ...
    // 修复网站管理子菜单
	private void repairMenuWzGl(Symenu wzgl) {
		Symenu ztgl = new Symenu();
		ztgl.setId("ztgl");
		ztgl.setText("专题管理");
		ztgl.setSrc("");
		ztgl.setSymenu(wzgl);
		ztgl.setSeq(BigDecimal.valueOf(0));
        repairMenuSubZtGl(ztgl);
		menuDao.saveOrUpdate(ztgl);

		Symenu sjdr = new Symenu();
		sjdr.setId("sjdr");
		sjdr.setText("数据导入");
		sjdr.setSrc("dataImportController.do?main");
		sjdr.setSymenu(wzgl);
		sjdr.setSeq(BigDecimal.valueOf(0));
		menuDao.saveOrUpdate(sjdr);

	}

    private void repairMenuSubZtGl(Symenu ztgl){
        Symenu subSZtGl = new Symenu();
        subSZtGl.setId("subZtGl");
        subSZtGl.setText("专题界面");
        subSZtGl.setSrc("/topicController.do?main");
        subSZtGl.setSymenu(ztgl);
        subSZtGl.setSeq(BigDecimal.valueOf(0));
        menuDao.saveOrUpdate(subSZtGl);

        Symenu subSZtSxGl = new Symenu();
        subSZtSxGl.setId("subZtSxGl");
        subSZtSxGl.setText("专题属性界面");
        subSZtSxGl.setSrc("/topicAttrController.do?main");
        subSZtSxGl.setSymenu(ztgl);
        subSZtSxGl.setSeq(BigDecimal.valueOf(0));
        menuDao.saveOrUpdate(subSZtSxGl);
    }
